...
> **Revisão DEC-07 (2ª sessão de bancada, 2026-09-12):** amostragem **1200 ms ± 150 ms** e polling da dashboard **2000 ms** (reflexão ≤ 2,5 s); leitura/conversão do DS18B20 **vinculadas à ROM** identificada na varredura; re-varredura corrigida (`getDeviceCount()` do `DallasTemperature` é cacheado — `rescan()` agora chama `begin()`). Evidências com sufixo `-dec07`/`-dec07b` (**ADR-014**: pull-up interno + retries de varredura/leitura).

## Resultados por critério de aceitação

| CA | Resultado | Evidência |
|---|---|---|
| CA-001 | PASS | ROM **identificada/vinculada no `setup`** (`[sensor] DS18B20 detectado (ROM 28FFE203B41605C2) parasita=0` a 0,28 s — `...-boot-dec07c.log`) e usada nas leituras; estabilidade com a mitigação ADR-014: 0 inválidas/0 ausências em 90 s com `ON` retido e aquecimento 41,5 → 66,1 °C (`...-soak-retry-dec07b.log`) |
...
| CA-006 | PASS | HOST (`test_control_policy`) + bancada via HTTP: `/on` → PWM 1023, `/off` → 0 (`2026-09-12-bancada-rede-keepalive.log`); console ON/OFF (`...-bateria-injecao.log`); **clique real nos botões** do dashboard na 3ª sessão (Ligar → PWM 1023 com aquecimento 41,2 → 47,9 °C; Desligar → PWM 0 — `...-dashboard-botao-dec07d.log`) |
...
6. **Barramento OneWire sem pull-up externo (causa raiz, 3ª sessão — `-dec07b`):** o core ESP8266 só habilita o pull-up interno em `INPUT_PULLUP` (`INPUT` não habilita) e a lib `OneWire` usa `INPUT`; com nível alto fraco/flutuante mediram-se ~30% dos ciclos com leitura inválida (13/90 s), falha da 1ª transação após o reset do MCU (scan do boot) e ausência falsa ocasional — cada falha desliga a carga (fail-safe) e recusa `ON`. **ADR-014**: pull-up interno do GPIO4 + 3 tentativas de varredura no boot / 2 por re-varredura + 1 retry de leitura ⇒ 0 inválidas/0 ausências em 90 s e boot vinculando a ROM no `setup` (0,28 s). A/B: `...-soak-baseline-dec07b.log` (antes) × `...-soak-pullup-interno-dec07b.log` e `...-soak-retry-dec07b.log` (depois).
7. **Período de amostragem confirmado: 1200 ms** (nunca 1,4 s) — `samp_ms=1200` e janelas [1127, 1273] ms sob carga; a conversão de 750 ms cabe folgada no ciclo (não é preciso alongar o período).
8. **A resistência aquece com `pwm=1023`**: ensaio com a carga ligada elevou a temperatura de 41,5 °C a 66,1 °C em 90 s (medida pelo próprio DS18B20) — o sintoma de "não ligar" era o fail-safe limpando o `ON` a cada falha de leitura, não a saída PWM (se não aquecer no hardware do usuário, verificar driver/carga em GPIO5).
9. **Botões do dashboard inoperantes (`cmd is not defined`) — 3ª sessão:** os 3 botões usavam `onclick` inline, mas `cmd` está dentro do IIFE do script (não é global) → clique lançava `Uncaught ReferenceError` e nada acontecia (o `ON` só funcionava por HTTP/console). Corrigido com `addEventListener` dentro do IIFE; validado em navegador real: Ligar → PWM 1023 (aquecimento 41,2 → 47,9 °C), Desligar → PWM 0, Rearmar → recusa `not_latched` com feedback (`2026-09-12-dashboard-botao-dec07d.log`).

## Limitações e pendências físicas

- **P3 (pull-up externo)**: recomendado 4,7 kΩ entre DQ (D2) e 3,3 V para margem de projeto; a mitigação em firmware (ADR-014) estabilizou o protótipo em bancada, mas o pull-up interno é fraco e não substitui o valor especificado de barramento.
