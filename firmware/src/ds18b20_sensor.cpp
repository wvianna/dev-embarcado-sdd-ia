...
void logAddress(const uint8_t* addr) {
  ...
  Serial.printf("[sensor] DS18B20 detectado (ROM %s)\n", buf);
}
...
bool enumerate(bool boot) {
  g_ds.begin();
  uint8_t found[8];
  if (g_ds.getDeviceCount() == 0 || !g_ds.getAddress(found, 0)) {
    if (g_present) {
...
Reading read() {
  ...
  // -127 (DEVICE_DISCONNECTED_C), NaN ou fora da faixa => inválida (FR-006/FR-014).
