...
**Robustez elétrica (ADR-014):** pull-up interno do GPIO4 habilitado antes da varredura; varredura com 3 tentativas imediatas no boot e 2 por re-varredura; leitura com 1 retry imediato em falha de comunicação; log da ROM inclui `parasita=` (0 = alimentação externa).
...
**ADR-014: Mitigação elétrica do OneWire (pull-up interno + tentativas/retry) e diagnóstico do barramento** | Contexto: o protótipo opera **sem pull-up externo** (P3 em aberto). O core ESP8266 só habilita o pull-up interno em `INPUT_PULLUP` (`INPUT` não habilita) e a biblioteca `OneWire` usa `INPUT` em todas as operações → nível alto fraco/flutuante. Medições de bancada (2ª sessão): ~30% dos ciclos com leitura inválida (13 falhas/90 s), falha do **scan no boot** (1ª transação após o reset do MCU falha; a 2ª, 5 ms depois, funciona: `devices=0` → `devices=1`) e ausência falsa ocasional na re-varredura; cada falha desliga a carga (fail-safe) e recusa `ON`. | Decisão: (1) habilitar o **pull-up interno** do GPIO4 antes da varredura; (2) **3 tentativas imediatas** de varredura no boot e **2** por re-varredura; (3) **1 retry imediato** na leitura de temperatura; (4) registrar `parasita=` no log da ROM. | Consequências: em bancada, 0 leituras inválidas e 0 ausências em 90 s com a carga ligada (`ON` retido; 41,5 → 66,1 °C de aquecimento), detecção da ROM no `setup` (0,28 s) e janela de amostragem [1145, 1255] ms. Sem mudança de semântica de segurança (o retry não altera a definição de leitura inválida — FR-006). **O resistor externo de 4,7 kΩ continua recomendado (P3)**: o pull-up interno é fraco e não substitui o valor especificado de barramento.
