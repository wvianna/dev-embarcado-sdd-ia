...
|---|---|---|---|---|---|---|
| FR-001 | CA-001, CA-002, CA-017 | M2–M3 | `src/ds18b20_sensor`, `src/main.cpp` | `firmware/test/test_control_policy` (estado de boot) + log serial | HOST + BANCADA em `docs/05-testing/` | PENDENTE — detecção de ROM requer sensor físico no D2; ausência observada e logada |
...
| FR-003 | CA-003 | M2–M3 | `src/ds18b20_sensor`, `lib/thermal_logic/periodic_timer` | `firmware/test/test_periodic_timer` + desconexão física | HOST + BANCADA em `docs/05-testing/` | PENDENTE — reconexão em runtime requer sensor físico |
| FR-004 | CA-004, CA-021 | M1–M3 | `lib/thermal_logic/periodic_timer` | `firmware/test/test_periodic_timer` + medição de ciclo | HOST + BANCADA em `docs/05-testing/` | PASS (HOST+BANCADA — janela 999–1001 ms sob polling) |
| FR-005 | CA-004, CA-005 | M2–M3 | `src/ds18b20_sensor` | log de ciclo em bancada (insp. de chamadas assíncronas) | BANCADA em `docs/05-testing/` | PASS (inspeção + estabilidade da amostragem em bancada) |
...
| FR-014 | CA-015, CA-017 | M1–M3 | `lib/thermal_logic/control_policy`, `lib/thermal_logic/status_json` | `firmware/test/test_control_policy`, `firmware/test/test_status_json` | HOST + BANCADA em `docs/05-testing/` | PASS (HOST + injeção `FAULT`; variante com desconexão física PENDENTE) |
...
| FR-029 | CA-023 | M2–M3 | `src/web/dashboard_html.h`, `src/web_server` | observação de atualização em bancada | BANCADA em `docs/05-testing/` | PASS (sem recarga; ≤ 2 s; marcador de sessão preservado) |
...
| NFR-003 | CA-004, CA-013, CA-021 | M1–M3 | `lib/thermal_logic/periodic_timer`, `lib/thermal_logic/buzzer_pattern`, `src/main.cpp` | `firmware/test/test_periodic_timer`, `firmware/test/test_buzzer_pattern` + medição | HOST + BANCADA em `docs/05-testing/` | PASS* (amostragem 999–1001 ms; buzzer HOST; audição PENDENTE) |
